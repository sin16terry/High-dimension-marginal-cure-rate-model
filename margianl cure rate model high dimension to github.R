...
while (m <= MC) {
  ...
  while (CONTINUE) {
    ...
    #####################################################################################
    #############PART3(start the quadratic approximation & CD algorithm)#################
    #####################################################################################
    continue = TRUE   #
    iter = 0
    ...
    while (continue) {
      ...
      #### calculate quantities needed for the quadatic programming##
      eta = trainx %*% beta[-(1:2)]
      sum(llder(eta))
      h = 0.001
...
#####################################################################################
#########################PART3(output results of interest)###########################
#####################################################################################
...
mean(ave.uncensor)
mean(mse)              #####output estimation bias
mean(std.est[c(3:8)])  #####output standard deviation of estimation
mean(False.Post.Ra.1)  #####output false positive rate
mean(False.Nega.Ra.1)  #####output false negative rate
mean(False.Post.Ra.05)
mean(False.Nega.Ra.05)
print(para.estimation) #####output average estimation for MC times of replication
#write.csv(para.estimation,'healthstudy2.csv')
#fix(para.estimation)
print(count.posi)     #####output the selected times for each covariate
